...
##########Backlog Items##############
# 1. Fix the duplicate teams logic
# 2. Format the tables that output the schedule
# 3. Functionality to limit the number of home games per town
# 4. Consider option to extend the season to accomodate more teams getting a game when they are uneven
...
def makeSchedule(seas, yr, leag, sd, ng, rs):
  ...
  for i in range(1, (ng + (totalTeams % 2) + 1)):
      ...
      if (totalTeams % 2 == 1):
          ...
          firstChoiceTeams = set([x for x in teamList if x not in byeTeams.values]).intersection(teamswMinHomeGames.values)
      else:
      ...
      if (len(firstChoiceTeams) < gamesPerGameDay):
          ...
          if (len(secondChoiceTeams) < homeGameDiff):
              ...
              tiTemp = set(pd.DataFrame(thirdChoiceTeams).sample(n=homeGameDiff2, random_state=rs, replace=False).squeeze(axis=1))
              ...
              tiTemp = set(pd.DataFrame(secondChoiceTeams).sample(n=homeGameDiff, random_state=rs, replace=False).squeeze(axis=1))
      ...
      for i2 in range(0, len(homeTeams)):
          ...
          if i2 < len(awayTeams):
              for j2 in range(i2+1, len(awayTeams)):
                  if awayTeams[j2] in str(ptstr) and re.sub('[^A-Za-z]+', '', awayTeams[j2]) != re.sub('[^A-Za-z]+', '', homeTeams[i2]):
          ...
          if endFlag1 == 0:
              if i2 > 1:
                  for j3 in range(0, i2):
                      if awayTeams[j3] in str(ptstr) and re.sub('[^A-Za-z]+', '', awayTeams[j3]) != re.sub('[^A-Za-z]+', '', homeTeams[i2]):
          ...
          if (dupTeam == 1 and sameTown ==0):
              ...
              if i2 < len(awayTeams):
                  for k2 in range(i2+1, len(awayTeams)):
                      if awayTeams[k2] in str(ptstr) and re.sub('[^A-Za-z]+', '', awayTeams[k2]) != re.sub('[^A-Za-z]+', '', homeTeams[i2]):
              ...
              if endFlag2 == 0:
                  if i2 > 1:
                      for k3 in range(1, i2-1):
                          if awayTeams[k3] in str(ptstr) and re.sub('[^A-Za-z]+', '', awayTeams[k3]) != re.sub('[^A-Za-z]+', '', homeTeams[i2]):
          ...
          if (dupTeam == 0 and sameTown ==1):
              ...
              if i2 < len(awayTeams):
                  for l2 in range(i2+1, len(awayTeams)):
                      if awayTeams[l2] in str(ptstr) and re.sub('[^A-Za-z]+', '', awayTeams[l2]) != re.sub('[^A-Za-z]+', '', homeTeams[i2]):
              ...
              if endFlag3 == 0:
                  if i2 > 1:
                      for l3 in range(1, i2-1):
                          if awayTeams[l3] in str(ptstr) and re.sub('[^A-Za-z]+', '', awayTeams[l3]) != re.sub('[^A-Za-z]+', '', homeTeams[i2]):
      ...
      for j in range(0, len(homeTeams)):
          ...
          teamTrack.loc[teamTrack['team'] == homeTeams[j], 'playedTeams'] = teamTrack.loc[teamTrack['team'] == homeTeams[j], 'playedTeams'] + " " + teamTrack[teamTrack['team'] == awayTeams[j]]['team'].values[0]
      for k in range(0, len(awayTeams)):
          teamTrack.loc[teamTrack['team'] == awayTeams[k], 'awayGames'] += 1
          teamTrack.loc[teamTrack['team'] == awayTeams[k], 'playedTeams'] = teamTrack.loc[teamTrack['team'] == awayTeams[k], 'playedTeams'] + " " + teamTrack[teamTrack['team'] == homeTeams[k]]['team'].values[0]
  ...
  def make_pretty(styler):
      styler.set_caption("Full Schedule")
      #styler.format(homeTeam)
      styler.format_index(lambda v: v.strftime("%A"))
      styler.background_gradient(axis=None, vmin=1, vmax=totalTeams, cmap="YlGnBu")
      return styler
...
# Main Panel Design
if schedButton:
    makeSchedule(season, year, league, startDay, numberGames, randomSeed)
